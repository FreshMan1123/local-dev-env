...
DOCKER_IMAGE_NAME="crpi-phrybi23i96jrlni.cn-guangzhou.personal.cr.aliyuncs.com/app12311/local-dev-env:v1.9.3"

# 创建一个函数来执行容器运行前的准备工作
LOCAL_DEV_FUNCTION="local-dev-env() {
  # 检查当前目录是否存在init-data和test-data
  if [ ! -d \"\$(pwd)/init-data\" ] || [ ! -d \"\$(pwd)/test-data\" ]; then
    echo \"🚀 首次运行，正在初始化数据目录...\"
    # 创建临时容器来复制模板文件
    docker run --rm -v \"\$(pwd):/tmp/host\" $DOCKER_IMAGE_NAME cp -r /workspace/template/init-data /tmp/host/
    docker run --rm -v \"\$(pwd):/tmp/host\" $DOCKER_IMAGE_NAME cp -r /workspace/template/test-data /tmp/host/
    echo \"✅ 数据目录初始化完毕！\"
    echo \"   您可以直接在 'init-data' 和 'test-data' 文件夹中管理 SQL 脚本。\"
  fi
  # 设置环境变量并运行主容器
  docker run --rm -it -w /workspace -v /var/run/docker.sock:/var/run/docker.sock -v \"\$(pwd)\":/workspace -e HOST_PWD=\"\$(pwd)\" $DOCKER_IMAGE_NAME \"\$@\"
}"
...
if [ -n "$ZSH_VERSION" ]; then
  ...
  echo "\n${LOCAL_DEV_FUNCTION}\nalias ${ALIAS_NAME}='local-dev-env'\n"
  exit 1
fi
...
echo "$LOCAL_DEV_FUNCTION" >> "$CONFIG_FILE"
echo "alias ${ALIAS_NAME}='local-dev-env'" >> "$CONFIG_FILE"
echo "✅ 安装/更新成功！"

echo "\n🎉 请重启您的终端，或执行 'source ${CONFIG_FILE}' 来让命令立即生效。" 